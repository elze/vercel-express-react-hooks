...
app.get('/api/students', (req, res) => {
	try {			
		/*
		let students = [];
		Object.keys(allStudents).forEach(studentName => {		
		//console.log(`/api/terms: primarySkill = ${primarySkill} termsData.allTerms[primarySkill] = ${JSON.stringify(termsData.allTerms[primarySkill])}`);
			student.totalCount = allStudents[studentName].categories.length;
			let categories_sorted = allStudents[studentName].categories.sort((a, b) => parseInt(b.filesAndPhrases.length) - parseInt(a.filesAndPhrases.length));				
			allStudents[studentName].categories = categories_sorted;
			allStudents[studentName].showSecondary = false;
			students.push({primary_term: studentName, categories: allStudents[studentName].categories});
		});
		*/
